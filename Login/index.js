...
createDatabase("assets", "CREATE TABLE assets(asset_id VARCHAR(36) PRIMARY KEY, assets INT NOT NULL)", con);
...
createDatabase("assetsgroupmemberpair", "CREATE TABLE assetsgroupmemberpair(assetsgroup_id VARCHAR(36), member_id VARCHAR(36), targetasset INT NOT NULL, currentasset INT NOT NULL, PRIMARY KEY(assetsgroup_id, member_id), FOREIGN KEY (assetsgroup_id) REFERENCES assetsgroups(assetsgroup_id), FOREIGN KEY(member_id) REFERENCES members(member_id))", con)
createDatabase("assetmanagerpair", "CREATE TABLE assetmanagerpair(member_id VARCHAR(36), asset_id VARCHAR(36), category_id VARCHAR(36), PRIMARY KEY(member_id, asset_id, category_id), FOREIGN KEY(member_id) REFERENCES members(member_id), FOREIGN KEY (asset_id) REFERENCES assets(asset_id), FOREIGN KEY (category_id) REFERENCES categories(category_id))", con)
